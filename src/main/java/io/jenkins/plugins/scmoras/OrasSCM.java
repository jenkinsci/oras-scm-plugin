...
/**
 * SCM implementation that checks out the content of a repository packaged as an OCI artifact of
 * type {@code application/vnd.jenkins.repo.manifest.v1+json} and pulled with ORAS.
 */
public class OrasSCM extends SCM {
  ...
  public void checkout(
    ...
    listener
        .getLogger()
        .printf("Checking out %s with digest %s into %s%n", containerRef, digest, workspace);
  ...
  public PollingResult compareRemoteRevisionWith(
    ...
    String baselineDigest =
        baseline instanceof OrasSCMRevisionState state ? state.getDigest() : null;
    OrasSCMRevisionState remote = new OrasSCMRevisionState(remoteDigest);
    if (Objects.equals(remoteDigest, baselineDigest)) {
      listener
          .getLogger()
          .printf("No change found for %s, digest is still %s%n", containerRef, remoteDigest);
  ...
  static Registry buildRegistry(StandardUsernamePasswordCredentials credentials, boolean insecure) {
    ...
    if (credentials == null) {
      LOG.debug(
          "No credentials found for the container reference, will use default authentication");
      return builder.defaults().build();
    }
    LOG.debug(
        "Credentials found: {}, creating registry for username {}",
        credentials,
        credentials.getUsername());
    return builder
        .defaults(credentials.getUsername(), credentials.getPassword().getPlainText())
        .build();
  }
  ...
  /**
   * Pulls and extracts the repository artifact into the workspace. Runs on the node holding the
   * workspace (which may be a remote agent), so it must only rely on plain, serializable data.
   */
  private static final class PullTask implements ControllerToAgentFileCallable<Void> {
    ...
    public Void invoke(File workspace, VirtualChannel channel) throws IOException {
      ...
      registry.pullArtifact(
          ContainerRef.parse(containerRef), workspace.toPath(), OCI.PullOptions.overwrite());
      return null;
    }
  }
