...
/**
 * Provides {@link OrasSCMFileSystem} instances, enabling Pipeline's "lightweight checkout" for {@link OrasSCM}:
 * a single file (typically the Jenkinsfile) can be read directly from the packaged artifact without extracting
 * the whole repository to disk.
 */
@Extension
public class OrasSCMFileSystemBuilder extends SCMFileSystem.Builder {
    ...
    public SCMFileSystem build(
            @NonNull Item owner, @NonNull SCM scm, @CheckForNull SCMRevision rev, @CheckForNull Run<?, ?> build)
            throws IOException {
        if (!(scm instanceof OrasSCM oras)) {
            return null;
        }
        StandardUsernamePasswordCredentials credentials = OrasSCM.getCredentials(owner, oras.getCredentialsId());
        ...
        return new OrasSCMFileSystem(ref.withDigest(layer.getDigest()).toString(), oras.isInsecure(), credentials);
    }
